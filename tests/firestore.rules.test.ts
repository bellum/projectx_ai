...
describe('Firestore rules', () => {
  ...
  it('permits approved users to read and maintain the derived analytics summary only', async () => { const db = testEnv.authenticatedContext('approved', { calendarAccess: true }).firestore(), analytics = { schemaVersion: 1, periodCount: 1, latestPeriodId: 'a', latestStartedAt: valid.startedAt, latestEndedAt: valid.endedAt, intervals: [], updatedAt: valid.startedAt }; await assertSucceeds(setDoc(doc(db, 'periodAnalytics/summary'), analytics)); await assertSucceeds(getDoc(doc(db, 'periodAnalytics/summary'))); await assertFails(setDoc(doc(testEnv.unauthenticatedContext().firestore(), 'periodAnalytics/summary'), analytics)); await assertFails(setDoc(doc(db, 'periodAnalytics/other'), analytics)); await assertFails(setDoc(doc(db, 'periodAnalytics/summary'), { ...analytics, extra: true })) })
})
