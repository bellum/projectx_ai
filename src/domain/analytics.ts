...
export function buildPeriodAnalytics(periods: Period[]): PeriodAnalytics {
  ...
  return {
    ...
    intervals: sorted.slice(1).map((period, index) => ({ periodId: period.id, previousStart: sorted[index].start, start: period.start, end: period.end, intervalDays: daysBetween(sorted[index].end, period.start) })),
  }
}
