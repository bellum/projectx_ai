...
import type { GapSample, PeriodAnalytics } from '../types/period'
...
export function analyticsPayload(analytics: PeriodAnalytics) {
  return {
    ...
    gaps: analytics.gaps.map(sample => ({ periodId: sample.periodId, startedAt: isoDayToTimestamp(sample.start), endedAt: isoDayToTimestamp(sample.end), gapDays: sample.gapDays })),
...
function gap(value: unknown): GapSample {
  const raw = value as { periodId?: unknown; startedAt?: unknown; endedAt?: unknown; gapDays?: unknown }
  const gapDays = raw.gapDays
  if (typeof raw.periodId !== 'string' || !timestamp(raw.startedAt) || !timestamp(raw.endedAt) || typeof gapDays !== 'number' || !Number.isInteger(gapDays)) throw new Error('Malformed period analytics data.')
  return { periodId: raw.periodId, start: timestampToIsoDay(raw.startedAt as never), end: timestampToIsoDay(raw.endedAt as never), gapDays }
}
export function normalizeAnalytics(raw: unknown): PeriodAnalytics {
  const value = raw as { schemaVersion?: unknown; periodCount?: unknown; latestPeriodId?: unknown; latestStartedAt?: unknown; latestEndedAt?: unknown; gaps?: unknown }
  const periodCount = value.periodCount
  if (value.schemaVersion !== 1 || typeof periodCount !== 'number' || !Number.isInteger(periodCount) || periodCount < 0 || !Array.isArray(value.gaps)) throw new Error('Malformed period analytics data.')
  ...
  return { periodCount, latest: hasLatest ? { id: value.latestPeriodId as string, start: timestampToIsoDay(value.latestStartedAt as never), end: timestampToIsoDay(value.latestEndedAt as never) } : undefined, gaps: value.gaps.map(gap) }
}
