...
export function CalendarPage({ onInsights }: { onInsights(): void }) {
  ...
  return <main className="page"><header><div><p className="eyebrow">Shared calendar</p><h1>Periods</h1></div><div className="header-actions"><button type="button" className="secondary-action" onClick={onInsights}>Insights</button><button type="button" onClick={() => void signOut()} aria-label="Sign out">Sign out</button></div></header><p className="signed-in">Signed in as {user?.email ?? 'Google account'}</p>{loadError ? <p role="alert">Unable to load periods: {loadError}</p> : loading ? <p>Loading periods…</p> : <><p className="instruction">Tap an unmarked day to add a one-day period, or drag across dates to add a range. Tap a recorded period to edit it.</p><TwoMonthCalendar periods={periods} anchor={anchor} today={today} predicted={[predictions.latestInterval.date, predictions.average.date].filter((value): value is IsoDate => Boolean(value))} preview={preview} onAnchor={changeAnchor} onSelect={select} onRange={(start, end) => setDraft(createDraft(periods, start, end))} onRangePreview={setPreview}/><PredictionSummary {...predictions}/></>}{draft && <PeriodEditor draft={draft} today={today} pending={pending} error={mutationError} onSave={value => void execute(() => savePeriod(value))} onDelete={id => void execute(() => removePeriod(id))} onCancel={() => { setDraft(undefined); setMutationError(undefined) }}/>}</main>
}
