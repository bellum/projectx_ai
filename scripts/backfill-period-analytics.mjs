...
const gaps = periods.slice(1).map((period, index) => ({ periodId: period.id, startedAt: dayTimestamp(period.start), endedAt: dayTimestamp(period.end), gapDays: daysBetween(periods[index].end, period.start) }))
const latest = periods.at(-1)
await database.doc('periodAnalytics/summary').set({ schemaVersion: 1, periodCount: periods.length, ...(latest ? { latestPeriodId: latest.id, latestStartedAt: dayTimestamp(latest.start), latestEndedAt: dayTimestamp(latest.end) } : {}), gaps, updatedAt: Timestamp.now() })
console.log(JSON.stringify({ migratedPeriods: periods.length, storedGaps: gaps.length }))
